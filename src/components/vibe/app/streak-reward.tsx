                            ...
                            className={`grid place-items-center h-9 w-9 rounded-full text-xs font-bold transition ${
                              isClaimed
                                ...
                                : isToday && data.canClaim
                                  ? "bg-amber-400/20 ring-2 ring-amber-400 text-amber-600 dark:text-amber-300"
                                  : "v-surface-1 v-fg-muted ring-1 ring-[var(--v-divider)]"
                            }`}
                          >
                            {isClaimed ? <Check className="h-4 w-4" /> : r.day}
                          </motion.div>
                          <span className={`text-[8px] tabular-nums ${isClaimed ? "text-amber-600 dark:text-amber-300" : "v-fg-muted"}`}>
                            +{r.reward}
                          </span>
                        </div>
                      );
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        exit={{ scale: 0.85, opacity: 0, y: 20 }}
        transition={{ type: "spring", stiffness: 300, damping: 25 }}
        className="relative w-full max-w-sm rounded-3xl v-surface-solid ring-1 ring-[var(--v-divider)] p-6 shadow-2xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Glow background */}
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <button
            ...
            className="flex flex-col items-center gap-1 rounded-2xl v-surface-1 ring-1 ring-[var(--v-divider)] p-3 hover:v-surface-2 transition active:scale-95"
          >
            ...
            <span className="text-[10px] v-fg-muted">WhatsApp</span>
          </button>
          <button
          ...
          >
            ...
            <span className="text-[10px] v-fg-muted">Facebook</span>
          </button>
          <button
          ...
          >
            ...
            <span className="text-[10px] v-fg-muted">Twitter / X</span>
          </button>
        </div>
