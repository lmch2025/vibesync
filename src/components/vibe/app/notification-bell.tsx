  ...
  return (
    <>
      ...
      <button
        ...
        className="relative h-9 w-9 grid place-items-center rounded-full hover:bg-white/10 transition"
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 inset-x-0 z-[70] rounded-t-3xl bg-zinc-900 ring-1 ring-white/10 p-4 pb-6 max-h-[70vh] overflow-y-auto no-scrollbar"
            >
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-display font-bold text-lg">Notifications</h3>
                <button onClick={() => setOpen(false)} className="h-8 w-8 grid place-items-center rounded-full hover:bg-white/10">
                  <X className="h-4 w-4" />
                </button>
              </div>
                  ...
                  {notifications.map((n) => (
                    <div
                      ...
                      className={`flex items-start gap-3 rounded-2xl p-3 transition ${
                        n.read ? "bg-white/[0.02]" : "bg-white/5 ring-1 ring-white/10"
                      }`}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs ${n.read ? "text-white/40" : "text-white/70"}`}>
                          {n.body}
                        </p>
                        <p className="text-[9px] text-white/30 mt-0.5">{formatTime(n.createdAt)}</p>
                      </div>
                      {!n.read && (
              ...
              {notifications.length > 0 && unreadCount > 0 && (
                <button
                  onClick={markAllRead}
                  className="mt-3 w-full h-9 rounded-xl bg-white/5 ring-1 ring-white/10 text-xs text-white/60 hover:bg-white/10 transition flex items-center justify-center gap-1.5"
                >
